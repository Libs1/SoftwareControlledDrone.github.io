...
//App Joysticks
//volatile uint16_t throttle=0, rudder=0, aileron = 0, elevator = 0;
uint16_t throttle = 0;
uint16_t rudder = 0;
uint16_t aileron = 0;
uint16_t elevator = 0;
...
{  
      ...
      if(serialAvailable == 1)
      {
          if(Serial.available( ) > 4)
          {
              ...
              else
              {
                  ...
                  throttle = throttle * 3;
                  //throttle = Serial.write(throttle >> 8);
      ...
      if(serialAvailable == 0)
      {
        ...
        if(reset || ppm[AUX8] > PPM_MAX_COMMAND) {
              ...
              init_protocol();
              Serial.println("init_protocol complete");
              Serial.println("Physical Joysticks");
          }
...
void update_ppm2()
{
  //ppm[THROTTLE] = map(throttle, 0, 1023, 1000, 2000);
  ppm[THROTTLE] = throttle;
  //ppm[RUDDER] = map(rudder, 0, 1023, 1000, 20000);
  ppm[RUDDER] = rudder;
  ppm[RUDDER] = 3000-ppm[RUDDER];
  //ppm[ELEVATOR] = map(elevator, 0, 1023, 1000, 2000);
  ppm[ELEVATOR] = elevator; 
  //ppm[AILERON] = map(aileron, 0, 1023, 1000, 2000);
  ppm[AILERON] = aileron;
  //ppm[THROTTLE] = 3000-ppm[THROTTLE];
  //ppm[ELEVATOR] = 3000-ppm[ELEVATOR];
}
