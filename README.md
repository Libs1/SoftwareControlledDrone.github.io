...
following company/department Future Humber Quadcopter Swarm. In the winter
semester I plan to form a group with Kevin Libdan, who is also building similar
hardware this term and working on the mobile application with me. The hardware
...
The problem solved by project is to control a drone remotely with an application
built for android devices. The problem is to recognize the radio signal used to
communicate with the drone and make the android application to send proper
flight instructions to the drone.
